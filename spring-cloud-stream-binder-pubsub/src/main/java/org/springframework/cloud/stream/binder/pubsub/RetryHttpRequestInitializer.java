...
public class RetryHttpRequestInitializer implements HttpRequestInitializer {
	 ...
	 * A private logger.
	 */
	private static final Logger LOG = Logger.getLogger(RetryHttpRequestInitializer.class
			.getName());
	 ...
	 * recovering from certain unsuccessful error codes wherein the Credential must
	 * refresh its token for a retry.
	 */
	 ...
	 * A constructor.
	 *
	 * @param wrappedCredential Credential which will be wrapped and used for providing
	 * auth header.
	 */
	public RetryHttpRequestInitializer(final Credential wrappedCredential) {
	 ...
	 */
	RetryHttpRequestInitializer(final Credential wrappedCredential, final Sleeper sleeper) {
	...
	public final void initialize(final HttpRequest request) throws IOException {
		...
		request.setUnsuccessfulResponseHandler(new HttpUnsuccessfulResponseHandler() {
			@Override
			public boolean handleResponse(final HttpRequest request,
					final HttpResponse response, final boolean supportsRetry)
					throws IOException {
